  ...
  <b-row>
    <b-col>
      <b-card no-body class="mb-3">
        <b-card-header>
          <font-awesome-icon icon="table"/> Profile Results
          <div v-bind:title="testBind"></div>
        </b-card-header>
        <b-card-body>
          <table id="controls_table"></table>
        </b-card-body>
      </b-card>
    </b-col>
  </b-row>
...
/* http://prismjs.com/download.html?themes=prism&languages=markup+css+clike+javascript+ruby+git+json */
...
export default {
  ...
  mounted() {
    ...
    vm.dtHandle = $('#controls_table').DataTable({
      ...
      info: false,
      columnDefs: [
        { width: 50, targets: 1}, { width: 600, targets: 2 }
      ],
    });
    val.forEach(function (item) {
      vm.dtHandle.rows.add($(helpers.rowHtml(item))).draw();
    ...
    $('#controls_table tbody').on( 'click', 'tr td.details-control', function () {
